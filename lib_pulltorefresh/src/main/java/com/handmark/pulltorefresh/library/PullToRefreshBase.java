...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
		...
		final T refreshableView = mRefreshableView;
	...
	//TODO 배경 지정.
	/**
	 * Set the background used in the loading layout. This is the same as calling
	 * <code>setBackgroundDrawable(drawable, Mode.BOTH)</code>
	 * 
	 * @param drawable
	 *            - Drawable to display on background
	 */
	public void setBackgroundDrawable(Drawable drawable) {
		setBackgroundDrawable(drawable, Mode.BOTH);
	}

	/**
	 * Set the background used in the loading layout.
	 * 
	 * @param drawable
	 *            - Drawable to display on background
	 * @param mode
	 ...
	 */
	public void setBackgroundDrawable(Drawable drawable, Mode mode) {
		if (null != mHeaderLayout && mode.canPullDown()) {
			if(android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN){
				mHeaderLayout.setBackground(drawable);
			}else{
		...
		if (null != mFooterLayout && mode.canPullUp()) {
			if(android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN){
				mFooterLayout.setBackground(drawable);
			}else{
				mFooterLayout.setBackgroundDrawable(drawable);
			}
		}
	...
	//TODO 배경 지정. 모양이 아쉽다. 저기 위쪽으로 전부 색이 지정되는게 아니라 예쁘지 않아.. repaint하긴 그렇구..
		/**
		 * Set the background used in the loading layout. This is the same as calling
		 * <code>setBackgroundDrawable(drawable, Mode.BOTH)</code>
		 * 
		 * @param resId
		 *            - Resource id to display on background
		 */
		public void setBackgroundResId(int resId) {
			setBackgroundResId(resId, Mode.BOTH);
		}
		 ...
		 *            will update the relevant View.
		 */
		public void setBackgroundResId(int resId, Mode mode) {
			if (null != mHeaderLayout && mode.canPullDown()) {
				mHeaderLayout.setBackgroundResource(resId);
			}
			if (null != mFooterLayout && mode.canPullUp()) {
				mFooterLayout.setBackgroundResource(resId);
			}
	...
	public void setLongClickable(boolean longClickable) {
		mRefreshableView.setLongClickable(longClickable);
	}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (a.hasValue(R.styleable.PullToRefresh_ptrHeaderBackground)) {
			...
			//if (null != background) {
				setBackgroundResource(R.styleable.PullToRefresh_ptrHeaderBackground);
			//}
		}
		if (a.hasValue(R.styleable.PullToRefresh_ptrAdapterViewBackground)) {
			//Drawable background = a.getDrawable(R.styleable.PullToRefresh_ptrAdapterViewBackground);
			//if (null != background) {
				mRefreshableView.setBackgroundResource(R.styleable.PullToRefresh_ptrAdapterViewBackground);
			//}
		}
		a.recycle();
		a = null;
	...
	private boolean isReadyForPull() {
		//TODO 일단 임시방편으로 새로고침중일땐 빨려들어오지 못하게 고쳤다.
		// 다만, BOTH일땐 어떡해야하는가.... 다른 하나가 하고 있으면 나머지 하나는 할 수 가 없당..
		if(mShowViewWhileRefreshing){
			if(isRefreshing()){
				return false;
			}
